...
Prove Theorem \ref{2.2.6}, possible with some absolute constant instead of 2 in the tail.
\end{ex}
\begin{proof}[Answer]
\end{proof}

\begin{ex}{2.2.8}[Boosting randomized algorithms]\label{2.2.8}
Suppose we have a randomized algorithm for a decision problem that is correct with probability $1/2 + \eps$ for some $\eps>0$. Show that running the algorithm $N$ times independently and taking the majority yields the correct answer with probability $\geq 1 -\delta$ for $N\geq \frac 1 {2\epsilon^2} \log \frac 1 \delta$
\end{ex}
\begin{proof}[Answer]
\end{proof}
